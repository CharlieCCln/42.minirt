...
/*   Updated: 2023/09/26 19:04:03 by cgelin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	caps_intersection(t_ray ray, const t_object cylinder)
{
	double	id1;
	double	id2;
	t_coords	ip1;
	...
	c2 = v_oper(ADD, cylinder.origin, v_scale(cylinder.dir, cylinder.height));
	id1 = solve_plane(ray.origin, ray.dir, cylinder.origin, cylinder.dir);
	id2 = solve_plane(ray.origin, ray.dir, c2, cylinder.dir);
	if (id1 < INFINITY || id2 < INFINITY)
	{
		ip1 = v_oper(ADD, ray.origin, v_scale(ray.dir, id1));
		ip2 = v_oper(ADD, ray.origin, v_scale(ray.dir, id2));
		if ((id1 < INFINITY && distance(ip1, cylinder.origin) <= cylinder.diameter) && (id2 < INFINITY && distance(ip2, c2) <= cylinder.diameter))
			return (id1 < id2 ? id1 : id2);
		else if (id1 < INFINITY && distance(ip1, cylinder.origin) <= cylinder.diameter)
			return (id1);
		else if (id2 < INFINITY && distance(ip2, c2) <= cylinder.diameter)
			return (id2);
	}
	return (INFINITY);
}
...
static bool	solve_cylinder(double x[2], t_ray ray, const t_object cylinder)
{
	...
	u = v_scale(cylinder.dir, v_dot(v_oper(SUB, ray.origin, cylinder.origin), cylinder.dir));
...
static void		calc_cy_normal(double x2[2], t_object cylinder, double dist1, double dist2)
{
	...
	x2[0] = x;
	// return (normalize(v_oper(SUB, v_oper(SUB, v_scale(ray.dir, x),	v_scale(cylinder.dir, dist)), v_oper(SUB, cylinder.origin, ray.origin))));
}
...
static double	cy_intersection(t_ray ray, t_object cylinder)
{
	...
	if (!solve_cylinder(x2, ray, cylinder))
		return (INFINITY);
	double dist1 = v_dot(cylinder.dir, v_oper(SUB, v_scale(ray.dir, x2[0]), v_oper(SUB, cylinder.origin, ray.origin)));
	double dist2 = v_dot(cylinder.dir, v_oper(SUB, v_scale(ray.dir, x2[1]), v_oper(SUB, cylinder.origin, ray.origin)));
...
int	intersect_cylinder(t_ray *ray, t_object *cyl)
{
	...
	if (ray->dist > hit_dist && hit_dist > EPSILON)
	{
		...
		if (is_on_side)
		{
			...
			ray->hit_norm = v_oper(SUB, get_closest_point_from_line(cyl->origin, pos, ray->hit), ray->hit);
		}
		else
