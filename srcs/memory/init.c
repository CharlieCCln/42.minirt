...
/*   Updated: 2023/09/12 15:17:02 by charlie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	allocate_objects(t_data *data, int *counts)
{
	...
	while (i < counts[3])
	{
		ft_bzero(&data->objects[i].origin, sizeof(t_coords));
		ft_bzero(&data->objects[i].dir, sizeof(t_coords));
...
void	bzero_everything(t_data *data)
{
	...
	ft_bzero(&data->light.origin, sizeof(t_coords));
